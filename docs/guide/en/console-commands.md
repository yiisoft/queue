...
> **Note:** `queue:run` and `queue:listen-all` use `QueueConsumerProviderInterface::getConsumerQueues()` when no queue names are passed. Explicitly passed names are resolved with `getConsumer()` and must have a consumer role.
...
```sh
yii queue:run [queue1 [queue2 [...]]] --limit=100
```

## 2. Listen for queued messages and process them continuously

The following command launches a daemon, which infinitely consumes messages from a single queue. This command receives an optional `queue` argument to specify which queue to listen to, defaults to the queue name `yii-queue`.
...
- `queue` argument(s). Specify one or more queue names to process. Messages from other queues will be ignored. Defaults to all registered consumer-capable queue names.
...
The full command signature is:
```sh
yii queue:listen-all [queue1 [queue2 [...]]] --pause=1 --limit=0
```
